...
static void editor_render_game_viewport()
{
    ...
    for (int y = 0; y < ROOM_HEIGHT; y++)
    {
        for (int x = 0; x < ROOM_WIDTH; x++)
        {
            ...
            int editor_tile_size = 32;
            render_rect(vp_x + x * editor_tile_size, vp_y + y * editor_tile_size,
                        editor_tile_size, editor_tile_size, color);

            // Tile borders for grid
            if (g_editor.show_grid)
            {
                render_rect_outline(vp_x + x * editor_tile_size, vp_y + y * editor_tile_size,
                                    editor_tile_size, editor_tile_size, 0xFF202020);
            }
        }
    }
...
{
    if (!g_editor.inspector.is_visible)
        return;
...
{
    if (!g_editor.console.is_visible)
        return;
