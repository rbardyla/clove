...
typedef struct {
    ...
    // Emotional update timing
    f32 last_emotion_update;  // Time since last emotional update
} npc;
...
typedef struct {
    ...
    Display* display;
    Window window;
    Pixmap backbuffer;  // Double buffer for smooth rendering
    GC gc;
    int screen;
...
void create_backstory(npc* n, const char* name, int personality_seed) {
    ...
    // Initialize emotional update timing
    n->last_emotion_update = 0.0f;  // Start immediately
...
void update_npc_mind(npc* n, f32 dt, game_state* game) {
    // Throttle emotional updates - only update every 2-3 seconds for realistic behavior
    const f32 EMOTION_UPDATE_INTERVAL = 2.5f;  // Seconds between emotional updates
    
    if (game->game_time - n->last_emotion_update < EMOTION_UPDATE_INTERVAL) {
        // Skip emotional processing, just update physical movement
        return;
    }
    
    // Time for an emotional update
    n->last_emotion_update = game->game_time;
    ...
    for (int i = 0; i < EMO_COUNT; i++) {
        ...
        // Much slower, more realistic emotional changes
        f32 decay_rate = 0.02f;  // Only 2% change per update (every 2.5 seconds)
        f32 current_diff = n->emotions[i] - baseline;
        
        // Emotional inertia - resist rapid changes
        if (current_diff > 0.1f) {
            n->emotions[i] -= decay_rate * (current_diff * 0.5f);  // Slower decay when far from baseline
        } else if (current_diff < -0.1f) {
            n->emotions[i] += decay_rate * (-current_diff * 0.5f);
        } else {
            // Small adjustments near baseline
            n->emotions[i] += (baseline - n->emotions[i]) * 0.1f;
        }
        
        // Apply much gentler momentum
        n->emotions[i] += n->emotional_momentum[i] * 0.3f;  // Reduced momentum impact
        n->emotional_momentum[i] *= 0.8f;  // Faster momentum decay for stability
    ...
    // Energy and stress affect emotions more gradually (no dt since we're throttled)
    if (n->energy < 0.3f) {
        n->emotions[EMO_ANGRY] += 0.01f;     // Small incremental changes
        n->emotions[EMO_HAPPY] -= 0.01f;
    }
    
    if (n->stress > 0.7f) {
        n->emotions[EMO_AFRAID] += 0.015f;   // Slightly larger for fear/stress
        n->emotions[EMO_ANGRY] += 0.01f;
    }
...
void render_game(game_state* game) {
    // Clear backbuffer each frame (double buffering eliminates flicker)
    ...
    if (game->dialog_active) {
        ...
        XFillRectangle(game->display, game->backbuffer, game->gc, 30, 520, 964, 180);
        
        XSetForeground(game->display, game->gc, nes_palette[0x30]);
        XDrawRectangle(game->display, game->backbuffer, game->gc, 30, 520, 964, 180);
    ...
    if (game->show_debug) {
        ...
        XSetForeground(game->display, game->gc, 0x000000);
        XFillRectangle(game->display, game->backbuffer, game->gc, 5, 50, 700, debug_height);
        
        // Draw border
        XSetForeground(game->display, game->gc, nes_palette[0x30]);
        XDrawRectangle(game->display, game->backbuffer, game->gc, 5, 50, 700, debug_height);
    ...
    // Copy backbuffer to front (double buffer swap) - eliminates flickering
...
int main() {
    ...
    // Create backbuffer for double buffering to eliminate flickering
    game->backbuffer = XCreatePixmap(game->display, game->window, 
                                   SCREEN_WIDTH, SCREEN_HEIGHT,
                                   DefaultDepth(game->display, game->screen));
    if (!game->backbuffer) {
        printf("Failed to create backbuffer\n");
